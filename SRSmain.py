...
def downloadRegCard(regRoot, width, height, userPHOTO):
    ...
    responce=messagebox.askyesno("MOSTRAR LA TARJETA DE REGISTRO???", "¿Quieres ver tu tarjeta de registro?")
    if responce==1:
        img.show()
        time.sleep(3)
    messagebox.showinfo("DESCARGADO EXITOSAMENTE!!!", "Su tarjeta de registro está aquí:\nFinalRegistrationCards\\"+picName)
...
def regCardClicked(root):
    ...
    if PermissionForUpdateOrDelete=="Granted":
        ...
        regRoot.title("TARJETA DE REGISTRO")
        ...
        messagebox.showwarning("¡¡¡ADVERTENCIA!!!", "Especifique los datos haciendo clic en el botón Mostrar")
...
def updateClicked(root, regE, rollE, fnameE, lnameE, photoPathEntry, finalAttr,sessionE, addressE, display, tEntityE, variable):
    ...
    if PermissionForUpdateOrDelete=="Granted":
        ...
        if preREG==newREG and preROLL==newROLL and preFNAME==newFNAME and preLNAME==newLNAME and newPHOTO==photoPathEntry.get() and preSESSION==newSESSION and preADDRESS==newADDRESS:
            #Nothing has changed!!!
            messagebox.showwarning("ADVERTENCIA!!!", "Los datos ya están en la base de datos!!!")
        else:
            ...
            if newPHOTO!=prePHOTO:
                os.remove(prePHOTO)
            KEY, PermissionForUpdateOrDelete=-1, "Denied"
            messagebox.showinfo("¡¡¡ACTUALIZADO EXITOSAMENTE!!!", "¡¡¡Sus datos se han actualizado exitosamente!!!")
...
def deleteClicked(root, regE, rollE, fnameE, lnameE, photoPathEntry, finalAttr,sessionE, addressE, display, tEntityE):
    global KEY, PermissionForUpdateOrDelete
    if PermissionForUpdateOrDelete=="Granted":
        responce=messagebox.askyesno("¿Estás seguro?", "¿Eliminarlo?")
        if responce==1:
            conn=sqlite3.connect("database\StudentDetails.db")
            c=conn.cursor()
            c.execute("SELECT *, oid FROM details")
            ...
            messagebox.showinfo("¡¡¡ELIMINADO!!!", "¡¡¡Eliminado con éxito!!!")
            KEY=-1
            PermissionForUpdateOrDelete="Denied"
            resetClicked(root, regE, rollE, fnameE, lnameE, photoPathEntry, finalAttr, sessionE, addressE, display, tEntityE)
    else:
...
def searchClicked(root, regE, rollE, fnameE, lnameE, photoPathEntry, finalAttr,sessionE, addressE, display, tEntityE):
    ...
    conn.close()
    if len(finalIDs)==0:
        messagebox.showwarning("¡¡¡ADVERTENCIA!!!", "¡¡¡No existe tal parámetro en el conjunto de datos!!!")
        resetClicked(root, regE, rollE, fnameE, lnameE, photoPathEntry, finalAttr, sessionE, addressE, display, tEntityE)
    else:
        conn=sqlite3.connect("database\StudentDetails.db")
        c=conn.cursor()
...
def displayClicked(root, regE, rollE, fnameE, lnameE, photoPathEntry, finalAttr,sessionE, addressE, display, tEntityE):
    ...
    try:
        ...
        messagebox.showwarning("¡¡¡ADVERTENCIA!!!", "¡¡¡Seleccione una línea del conjunto de datos existente!!!")
...
def addClicked(root, regE, rollE, fnameE, lnameE, photoPathEntry, finalAttr, variable, sessionE, addressE, tEntityE, display):
    ...
    sessionE.config(state="readonly")
    if  result=="NaN":
        messagebox.showerror("¡¡¡ERROR!!!", "Ingrese una sesión válida")
    else:
            ...
            if ("-" not in addressE.get().strip()) and ("/" not in addressE.get().strip()) and ('\\' not in addressE.get().strip()):
                try:
                    ...
                    if len(records)==0:
                        ...
                        messagebox.showinfo("¡¡¡AGREGADO EXITOSAMENTE!!!", "¡¡¡Los datos se agregaron exitosamente a la base de datos!!!")
                        conn.commit()
                        conn.close()
                        ...
                        for record in records:
                            if record[0]==regE.get().strip() and record[1]==rollE.get().strip() and record[2]==fnameE.get().strip() and record[3]==lnameE.get().strip() and record[5]==variable.get() and record[6]==addressE.get().strip():
                                messagebox.showerror("¡¡¡ERROR!!!", "¡¡¡Los datos ya están en la base de datos!!!")
                                conn.commit()
                                conn.close()
                                break
                        else:
                    ...
                    messagebox.showerror("¡¡¡ERROR!!!", "¡¡¡Proporcione una foto de 126 px X 126 px !!!")
            else:
                messagebox.showwarning("¡¡¡ADVERTENCIA!!!", "¡¡¡Proporcione la dirección sin ningún símbolo de guión ('-'), barra ('/') o barra invertida ('\\')!")
        else:
            messagebox.showerror("¡¡¡ERROR!!!", "¡¡¡Por favor proporcione toda la información!!!")
        conn=sqlite3.connect("database\StudentDetails.db")
        c=conn.cursor()
        c.execute("SELECT *, oid FROM details")
        records=c.fetchall()
...
def browseButtonClicked(root, photoPathEntry):
    ...
    try:
        ...
        if root.filename.endswith(".png") and width==126 and height==126:
            ...
            messagebox.showerror("¡¡¡ERROR!!!", "Seleccione un archivo PNG de tamaño 126px x 126px")
            photoPathEntry.config(state=NORMAL)
            photoPathEntry.delete(0, END)
            photoPathEntry.insert(0, "126px x 126px png")
            photoPathEntry.config(state=DISABLED)
...
def exitClicked(root):
    result=messagebox.askquestion("¡¡¡SALIDA!!!", "¿Quieres salir???")
    if result=="yes":
        root.destroy()
...
def main():
    ...
    root.title("SISTEMA DE REGISTRO DE ESTUDIANTES")
    ...
    reg=Label(inputFrame, text="REGISTRO NÚMERO   ", font=("arial b", 30), bg="cadet blue").grid(row=0, column=0)
    roll=Label(inputFrame, text="MATRICULA NÚMERO   ", font=("arial b", 30), bg="cadet blue").grid(row=1, column=0, pady=(20,0))
    fname=Label(inputFrame, text="NOMBRE   ", font=("arial b", 30), bg="cadet blue").grid(row=2, column=0, pady=(20,0))
    lname=Label(inputFrame, text="APELLIDO   ", font=("arial b", 30), bg="cadet blue").grid(row=3, column=0, pady=(20,0))
    photo=Label(inputFrame, text="FOTO  ", font=("arial b", 30), bg="cadet blue").grid(row=4, column=0, pady=(20,0))
    session=Label(inputFrame, text="SESIÓN   ", font=("arial b", 30), bg="cadet blue").grid(row=5, column=0, pady=(20,0))
    address=Label(inputFrame, text="DIRECIÓN   ", font=("arial b", 30), bg="cadet blue").grid(row=6, column=0, pady=(20,0))
    ...
    tEntity=Label(displayFrame, text="TOTAL ENTIDADES", font=("arial b", 30), bg="cadet blue").grid(row=0, column=0)
   ...
   #=========================================lower frame====================================
    ...
    resetB=Button(LFrame, text="REINICIAR", font=("Courier b", 20), bg="#01e779", cursor="hand2",
                  command=lambda:resetClicked(root,
                                              regE, rollE, fnameE, lnameE, photoPathEntry, finalAttr, sessionE, addressE, display,
                                              tEntityE
                                              )).grid(row=0,column=0)
    
    addB=Button(LFrame, text="AGREGAR", font=("Courier b", 20), bg="#0183f8", cursor="hand2",
    ...
    displayB=Button(LFrame, text="MOSTRAR", font=("Courier b", 20), bg="#a3a8a7", cursor="hand2",
                   command=lambda:displayClicked(root,
                                                 regE, rollE, fnameE, lnameE, photoPathEntry, finalAttr,sessionE, addressE, display,
                                                 tEntityE
                                                 )).grid(row=0,column=2, padx=(25,0))
    
    searchB=Button(LFrame, text="BUSCAR", font=("Courier b", 20), bg="#b69444", cursor="hand2",
                   command=lambda:searchClicked(root,
                                                regE, rollE, fnameE, lnameE, photoPathEntry, finalAttr,sessionE, addressE, display,
                                                tEntityE
                                                )).grid(row=0,column=3, padx=(25,0))
    
    updateB=Button(LFrame, text="ACTUALIZAR", font=("Courier b", 20), bg="yellow", cursor="hand2",
                   command=lambda:updateClicked(root,
                                                regE, rollE, fnameE, lnameE, photoPathEntry, finalAttr,sessionE, addressE, display,
                                                tEntityE, variable
                                                )).grid(row=0,column=4, padx=(25,0))
    
    deleteB=Button(LFrame, text="ELIMINAR", font=("Courier b", 20), bg="#7cd0ba", cursor="hand2",
                   command=lambda:deleteClicked(root,
                                                regE, rollE, fnameE, lnameE, photoPathEntry, finalAttr,sessionE, addressE, display,
                                                tEntityE
                                                )).grid(row=0,column=5, padx=(25,0))
    
    regCardB=Button(LFrame, text="GENERAR TARJETA DE REGISTRO", font=("Courier b", 20), bg="#c873b6", cursor="hand2",
                    command=lambda:regCardClicked(root)).grid(row=0,column=6, padx=(25,0))
    
    exitB=Button(LFrame, text="SALIR", font=("Courier b", 20), bg="#e36753", cursor="hand2",
